...
class RoomsFasilities(BaseRepository):
    ...
    async def set_room_fasilities(self, room_id: int, fasilities_ids: list[int]):
        query = select(self.model.fasilitie_id).filter_by(room_id=room_id)
        ...
        if to_delete:
            stmt = delete(self.model).filter(
                self.model.room_id == room_id, self.model.fasilitie_id.in_(to_delete)
            )
            await self.session.execute(stmt)

        if to_add:
            stmt = insert(self.model).values(
                [{"room_id": room_id, "fasilitie_id": f_id} for f_id in to_add]
            )
            await self.session.execute(stmt)
