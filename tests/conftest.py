...
async def add_data_in_database(setup_database):
    ...
    data_fasilities: list[FasilitiesAdd] = [
        FasilitiesAdd.model_validate(fasilitie) for fasilitie in dict_fasilities
    ]
    async with DBManager(session_factory=async_session_maker_null_pool) as db:
        await db.hotels.add_bulk(data_hotels)
        await db.rooms.add_bulk(data_rooms)
        await db.fasilities.add_bulk(data=data_fasilities)
        await db.commit()
...
async def register_user(add_data_in_database, ac):
    await ac.post(
        url="/users/register",
        json={"email": "user@e324xample.com", "password": "string"},
    )
...
def mock_redis(monkeypatch):
    monkeypatch.setattr(
        "src.connectors.redis_connector.RedisManager.set", AsyncMock(return_value=None)
    )
    monkeypatch.setattr(
        "src.connectors.redis_connector.RedisManager.get", AsyncMock(return_value=None)
    )
